 ...
 *   pi -p --session-control --control-session <session-name|session-id|project:name|tag:name|meta:key=value> --send-session-message <text>
 ...
 *   - { type: "set_name", name: "..." }
 *   - { type: "get_meta" }
 *   - { type: "set_meta", meta?: {...}, merge?: boolean, unset?: string[] }
 *   - { type: "get_message" }
 *   - { type: "get_summary" }
...
const SESSION_META_TYPE = "session-control-meta";
const SENDER_INFO_PATTERN = /<sender_info>[\s\S]*?<\/sender_info>/g;
const STALE_CONTROL_FILE_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
...
interface SessionControlMeta extends JsonObject {
	tags?: string[];
	status?: string;
	task?: string;
...
interface RpcInfoCommand {
	type: "info";
	id?: string;
}

interface RpcSetNameCommand {
	type: "set_name";
	name: string;
	id?: string;
}

interface RpcGetMetaCommand {
	type: "get_meta";
	id?: string;
}

interface RpcSetMetaCommand {
	type: "set_meta";
	meta?: JsonObject;
	merge?: boolean;
	unset?: string[];
...
function isSafeSessionId(sessionId: string): boolean {
	return !sessionId.includes("/") && !sessionId.includes("\\") && !sessionId.includes("..") && !/[\0\r\n]/.test(sessionId) && sessionId.length > 0;
}

function isSafeAlias(alias: string): boolean {
	return !alias.includes("/") && !alias.includes("\\") && !alias.includes("..") && !/[\0\r\n]/.test(alias) && alias.length > 0;
}
...
async function cleanupStaleControlFiles(maxAgeMs = STALE_CONTROL_FILE_MAX_AGE_MS): Promise<void> {
	...
	for (const entry of entries) {
		const entryPath = path.join(CONTROL_DIR, entry.name);
		let stat;
		try {
		...
		if (entry.name.endsWith(SOCKET_SUFFIX)) {
			...
			const alive = await isSocketAlive(entryPath);
			if (!alive) {
...
async function getLiveSessions(): Promise<LiveSessionInfo[]> {
	await ensureControlDir();
	await cleanupStaleControlFiles().catch(() => undefined);
	const entries = await fs.readdir(CONTROL_DIR, { withFileTypes: true });
...
function normalizeSessionMeta(value: unknown): SessionControlMeta {
	if (!value || typeof value !== "object" || Array.isArray(value)) return {};
	const input = value as JsonObject;
	const meta: SessionControlMeta = {};
	...
	for (const [key, raw] of Object.entries(input)) {
		...
		if (key === "status" || key === "task" || key === "projectOverride") {
...
function mergeSessionMeta(current: SessionControlMeta, patch?: JsonObject, unset?: string[], merge = true): SessionControlMeta {
	const next: SessionControlMeta = merge ? { ...current } : {};
	Object.assign(next, normalizeSessionMeta(patch ?? {}));
...
function loadSessionMetaFromBranch(ctx: ExtensionContext): SessionControlMeta {
	let meta: SessionControlMeta = {};
	for (const entry of ctx.sessionManager.getBranch()) {
		const maybeCustom = entry as { type?: string; customType?: string; data?: unknown };
		if (maybeCustom.type === "custom" && maybeCustom.customType === SESSION_META_TYPE) {
			meta = normalizeSessionMeta(maybeCustom.data);
		}
	}
	return meta;
}
...
function getLastActivityAt(ctx: ExtensionContext): number | undefined {
	const branch = ctx.sessionManager.getBranch();
	for (let i = branch.length - 1; i >= 0; i--) {
		const timestamp = parseEntryTimestamp(branch[i] as { timestamp?: string });
		if (timestamp !== undefined) return timestamp;
	}
	return undefined;
}

function getLastAssistantAt(ctx: ExtensionContext): number | undefined {
	const branch = ctx.sessionManager.getBranch();
	for (let i = branch.length - 1; i >= 0; i--) {
		const entry = branch[i];
		...
		const msg = entry.message;
		if ("role" in msg && msg.role === "assistant") {
			return typeof msg.timestamp === "number" ? msg.timestamp : parseEntryTimestamp(entry as { timestamp?: string });
		}
	}
	return undefined;
}

async function execTrim(pi: ExtensionAPI, command: string, args: string[]): Promise<string | null> {
	try {
...
async function detectProject(pi: ExtensionAPI, cwd: string, meta: SessionControlMeta): Promise<SessionProjectInfo> {
	const fallbackName = path.basename(cwd) || cwd;
	const overrideName = typeof meta.projectOverride === "string" && meta.projectOverride.trim()
		? meta.projectOverride.trim()
		: undefined;

	const root = await execTrim(pi, "git", ["-C", cwd, "rev-parse", "--show-toplevel"]);
	if (!root) {
		return {
			isGit: false,
			root: cwd,
	...
	const branch = await execTrim(pi, "git", ["-C", root, "branch", "--show-current"]);
	const commit = await execTrim(pi, "git", ["-C", root, "rev-parse", "--short", "HEAD"]);
	const relativePath = path.relative(root, cwd);

	return {
		isGit: true,
		root,
		name: overrideName ?? (path.basename(root) || root),
...
async function getAliasesForSocket(socketPath: string | null): Promise<string[]> {
	if (!socketPath) return [];
	try {
		const aliasMap = await getAliasMap();
		return aliasMap.get(socketPath) ?? [];
	} catch {
		return [];
	}
}
...
async function buildSessionInfo(
	...
	const socketPath = state.socketPath ?? getSocketPath(sessionId);
	const name = ctx.sessionManager.getSessionName()?.trim() || undefined;
	const cwd = ctx.sessionManager.getCwd() || ctx.cwd;
	const meta = normalizeSessionMeta(state.meta);
	const project = await detectProject(pi, cwd, meta);
	const aliases = await getAliasesForSocket(socketPath);
	...
	return {
		...
		state: {
			...
			lastActivityAt: getLastActivityAt(ctx),
			lastAssistantAt: getLastAssistantAt(ctx),
...
function fallbackSessionInfo(session: LiveSessionInfo): SessionInfoData {
	return {
		sessionId: session.sessionId,
		name: session.name,
		aliases: session.aliases,
		cwd: "",
		project: { isGit: false, root: "", name: "unknown" },
		state: { idle: false, hasPendingMessages: false, entryCount: 0 },
		meta: {},
	};
}

function isSessionInfoData(value: unknown): value is SessionInfoData {
	if (!value || typeof value !== "object") return false;
	const info = value as Partial<SessionInfoData>;
	return typeof info.sessionId === "string" && !!info.project && !!info.state;
}

async function fetchSessionInfo(session: LiveSessionInfo): Promise<SessionInfoData> {
	try {
		const result = await sendRpcCommand(session.socketPath, { type: "info" }, { timeout: 5000 });
		if (result.response.success && isSessionInfoData(result.response.data)) {
			return result.response.data;
		}
	} catch {
		// Fall through to the minimal legacy view.
	}
	return fallbackSessionInfo(session);
}

async function getLiveSessionInfos(): Promise<SessionInfoData[]> {
	const sessions = await getLiveSessions();
	const infos = await Promise.all(sessions.map((session) => fetchSessionInfo(session)));
	return infos.sort((a, b) => (a.name ?? a.sessionId).localeCompare(b.name ?? b.sessionId));
}
...
function sameTextFold(a: string | undefined, b: string | undefined): boolean {
...
function selectorHasNonIdFields(selector: SessionSelector): boolean {
	return !!(
...
function looksLikeSessionId(value: string): boolean {
	return /^[a-f0-9]{8,}(?:-[a-f0-9]+)*$/i.test(value.trim());
}

function parseControlTargetSelector(target: string): SessionSelector {
	const value = target.trim();
	const prefixed = value.match(/^(projectRoot|projectName|project|tag|status|sessionName|sessionId|meta):(.+)$/i);
	if (prefixed) {
		const key = prefixed[1].toLowerCase();
		const raw = prefixed[2].trim();
		if (key === "projectroot") return { projectRoot: raw };
		if (key === "projectname") return { projectName: raw };
		if (key === "project") return { project: raw };
		if (key === "tag") return { tag: raw };
		if (key === "status") return { status: raw };
		if (key === "sessionname") return { sessionName: raw };
		if (key === "sessionid") return { sessionId: raw };
		if (key === "meta") {
			const equals = raw.indexOf("=");
			if (equals === -1) return { metaKey: raw };
			return { metaKey: raw.slice(0, equals).trim(), metaValue: raw.slice(equals + 1).trim() };
		}
	}
	return looksLikeSessionId(value) ? { sessionId: value } : { sessionName: value };
}

function sessionMatchesSelector(info: SessionInfoData, selector: SessionSelector): boolean {
	if (selector.sessionId && info.sessionId !== selector.sessionId) return false;
	if (selector.sessionName) {
		const matchesName = sameText(info.name, selector.sessionName) || info.aliases.some((alias) => sameText(alias, selector.sessionName));
		if (!matchesName) return false;
	}
	if (selector.project) {
		const project = selector.project.trim();
		const matchesProject = sameTextFold(info.project.name, project) ||
			samePath(info.project.root, project) ||
			sameTextFold(typeof info.meta.projectOverride === "string" ? info.meta.projectOverride : undefined, project);
		if (!matchesProject) return false;
	}
	if (selector.projectName && !sameTextFold(info.project.name, selector.projectName)) return false;
	if (selector.projectRoot && !samePath(info.project.root, selector.projectRoot)) return false;
	if (selector.tag) {
		const tag = selector.tag.trim();
		const tags = info.meta.tags ?? [];
		if (!tags.some((item) => sameTextFold(item, tag))) return false;
	}
	if (selector.status && !sameTextFold(info.meta.status, selector.status)) return false;
	if (selector.metaKey) {
		const key = selector.metaKey.trim();
		if (!(key in info.meta)) return false;
		if (selector.metaValue !== undefined) {
			const raw = info.meta[key];
			if (String(raw) !== selector.metaValue.trim()) return false;
		}
	}
	return true;
}

function formatState(info: SessionInfoData): string {
	const parts = [info.state.idle ? "idle" : "busy"];
	if (info.state.hasPendingMessages) parts.push("pending");
	return parts.join(", ");
}

function formatSessionCandidate(info: SessionInfoData): string {
	const label = info.name ? `${info.name} (${info.sessionId})` : info.sessionId;
	const project = info.project.root ? `${info.project.name} @ ${info.project.root}` : info.project.name;
	return `${label} | project=${project} | state=${formatState(info)}`;
}

function formatSessionInfo(info: SessionInfoData, currentSessionId?: string): string {
	const label = info.name ?? info.sessionId;
	const current = currentSessionId && info.sessionId === currentSessionId ? " (current)" : "";
	const lines = [`- ${label}${current}`, `  id: ${info.sessionId}`];
	if (info.aliases.length > 0) lines.push(`  aliases: ${info.aliases.join(", ")}`);
	if (info.project.root) {
		let projectLine = `  project: ${info.project.name} (${info.project.isGit ? "git" : "dir"}) @ ${info.project.root}`;
		if (info.project.relativePath) projectLine += `/${info.project.relativePath}`;
		lines.push(projectLine);
	} else {
		lines.push(`  project: ${info.project.name}`);
	}
	if (info.project.branch || info.project.commit) {
		lines.push(`  git: ${[info.project.branch, info.project.commit].filter(Boolean).join(" @ ")}`);
	}
	if (info.cwd) lines.push(`  cwd: ${info.cwd}`);
	lines.push(`  state: ${formatState(info)}`);
	if (info.meta.status) lines.push(`  status: ${info.meta.status}`);
	if (info.meta.task) lines.push(`  task: ${info.meta.task}`);
	if (info.meta.tags?.length) lines.push(`  tags: ${info.meta.tags.join(", ")}`);
	if (info.model?.provider || info.model?.id) lines.push(`  model: ${[info.model.provider, info.model.id].filter(Boolean).join("/")}`);
	return lines.join("\n");
}

async function resolveTargetSession(selector: SessionSelector): Promise<{
	sessionId?: string;
	info?: SessionInfoData;
	error?: string;
	candidates?: SessionInfoData[];
}> {
	...
	if (sessionId) {
		...
		const alive = await isSocketAlive(socketPath);
		if (!alive) {
			return { error: `Target session not reachable: ${sessionId}` };
		}
		const info = await fetchSessionInfo({ sessionId, aliases: [], socketPath });
		if (selectorHasNonIdFields(selector) && !sessionMatchesSelector(info, { ...selector, sessionId: undefined })) {
			return { error: "Session id does not match the additional selector filters", candidates: [info] };
		}
		return { sessionId, info };
	}

	if (!selectorHasNonIdFields(selector)) {
		return { error: "Missing target selector (sessionId, sessionName, project/projectName/projectRoot, tag, status, or metaKey)" };
	}

	const matches = (await getLiveSessionInfos()).filter((info) => sessionMatchesSelector(info, selector));
	if (matches.length === 0) {
		return { error: "No live session matches the selector" };
	}
	if (matches.length > 1) {
		return { error: "Ambiguous session selector", candidates: matches };
	}
	return { sessionId: matches[0].sessionId, info: matches[0] };
}
...
async function handleCommand(
	...
	// Get rich live session info
	if (command.type === "info") {
		const info = await buildSessionInfo(pi, state, ctx);
		respond(true, "info", info);
		return;
	}

	// Set session display name (also updates the control alias)
	if (command.type === "set_name") {
		const name = typeof command.name === "string" ? command.name.trim() : "";
		if (!name) {
			respond(false, "set_name", undefined, "Missing name");
			return;
		}
		pi.setSessionName(name);
		await syncAlias(state, ctx);
		const info = await buildSessionInfo(pi, state, ctx);
		respond(true, "set_name", { name, info });
		return;
	}

	// Get custom session metadata
	if (command.type === "get_meta") {
		respond(true, "get_meta", { meta: normalizeSessionMeta(state.meta) });
		return;
	}

	// Set/merge custom session metadata
	if (command.type === "set_meta") {
		state.meta = mergeSessionMeta(state.meta, command.meta, command.unset, command.merge !== false);
		pi.appendEntry(SESSION_META_TYPE, state.meta);
		respond(true, "set_meta", { meta: state.meta });
		return;
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag(CONTROL_TARGET_FLAG, {
		description: "Target session name/id for startup control send; supports prefixes project:, projectName:, projectRoot:, tag:, status:, meta:key=value",
		type: "string",
	});
	...
	pi.on("session_start", async (_event, ctx) => {
		state.meta = loadSessionMetaFromBranch(ctx);
	...
	pi.on("session_info_changed", async (_event, ctx) => {
		await syncAlias(state, ctx);
		updateStatus(ctx, pi.getFlag(CONTROL_FLAG) === true);
	});
...
- info: Get rich live metadata for the session (name, cwd, git project, state, model, custom meta).
- set_name: Rename a session and update its session-control alias. Requires 'name'.
- get_meta: Get custom session-control metadata.
- set_meta: Merge or replace custom session-control metadata. Uses 'meta', 'merge', and 'unset'.
- clear: Rewind session to initial state.
- abort: Abort the current turn in the target session.

Target selection:
- sessionId: UUID of the session.
- sessionName: session name (alias from /name).
- project/projectName/projectRoot: target by detected git project. projectRoot is safest; projectName must be unique.
- tag/status/metaKey/metaValue: target by custom metadata set with action=set_meta.
- If a selector matches multiple live sessions, the tool returns candidates instead of guessing.
...
- One-shot startup send is available via extension flags:
  --session-control
  --control-session <session-name|session-id|project:name|projectRoot:path|tag:name|status:value|meta:key=value>
...
Messages automatically include sender session info for replies. When you want a response, instruct the target session to reply directly to the sender by calling send_to_session with the sender_info reference (do not poll get_message).`,
		parameters: Type.Object({
			...
			project: Type.Optional(Type.String({ description: "Target by project name, project root, or projectOverride metadata (must match exactly/uniquely)" })),
			projectName: Type.Optional(Type.String({ description: "Target by detected project/repository name (must be unique)" })),
			projectRoot: Type.Optional(Type.String({ description: "Target by detected git/root directory path" })),
			tag: Type.Optional(Type.String({ description: "Target by custom metadata tag" })),
			status: Type.Optional(Type.String({ description: "Target by custom metadata status" })),
			metaKey: Type.Optional(Type.String({ description: "Target sessions that have this custom metadata key" })),
			metaValue: Type.Optional(Type.String({ description: "When metaKey is set, require this stringified metadata value" })),
			action: Type.Optional(
				StringEnum(["send", "get_message", "get_summary", "info", "set_name", "get_meta", "set_meta", "clear", "abort"] as const, {
					description: "Action to perform (default: send)",
					default: "send",
				}),
			),
			name: Type.Optional(Type.String({ description: "New session name for action=set_name" })),
			meta: Type.Optional(Type.Object({}, { additionalProperties: true, description: "Metadata object for action=set_meta (e.g. { tags: ['pr:555'], status: 'running' })" })),
			merge: Type.Optional(Type.Boolean({ description: "For action=set_meta, merge into existing metadata (default true); false replaces metadata" })),
			unset: Type.Optional(Type.Array(Type.String(), { description: "Metadata keys to remove for action=set_meta" })),
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			const selector: SessionSelector = {
				...
				sessionName: params.sessionName?.trim(),
				project: params.project?.trim(),
				projectName: params.projectName?.trim(),
				projectRoot: params.projectRoot?.trim(),
				tag: params.tag?.trim(),
				status: params.status?.trim(),
				metaKey: params.metaKey?.trim(),
				metaValue: params.metaValue?.trim(),
			};
			const resolved = await resolveTargetSession(selector);
			if (!resolved.sessionId) {
				const candidates = resolved.candidates?.map((info) => `- ${formatSessionCandidate(info)}`).join("\n");
				const message = candidates ? `${resolved.error}:\n${candidates}` : (resolved.error ?? "Missing target session");
				return {
					content: [{ type: "text", text: message }],
					isError: true,
					details: { error: resolved.error, candidates: resolved.candidates },
				};
			}
			...
			const displayTarget = resolved.info?.name || selector.sessionName || selector.project || selector.projectName || selector.projectRoot || selector.tag || selector.status || targetSessionId;

			try {
				// Handle each action
				if (action === "info") {
					const result = await sendRpcCommand(socketPath, { type: "info" }, { timeout: 10000 });
					if (!result.response.success || !isSessionInfoData(result.response.data)) {
						return {
							content: [{ type: "text", text: `Failed: ${result.response.error ?? "invalid info response"}` }],
							isError: true,
							details: result,
						};
					}
					...
					return {
						content: [{ type: "text", text: formatSessionInfo(info) }],
						details: info,
					};
				}
				...
				if (action === "set_name") {
					...
					if (!name) {
						return {
							content: [{ type: "text", text: "Missing name for set_name action" }],
							isError: true,
							details: { error: "Missing name" },
						};
					}
					const result = await sendRpcCommand(socketPath, { type: "set_name", name }, { timeout: 10000 });
					...
					const data = result.response.data as { name?: string; info?: SessionInfoData };
					return {
						content: [{ type: "text", text: `Session named: ${data.name ?? name}` }],
						details: data.info ?? data,
					};
				}

				if (action === "get_meta") {
					const result = await sendRpcCommand(socketPath, { type: "get_meta" }, { timeout: 10000 });
					if (!result.response.success) {
						return {
							content: [{ type: "text", text: `Failed: ${result.response.error ?? "unknown error"}` }],
							isError: true,
					...
					const data = result.response.data as { meta?: SessionControlMeta };
					return {
				...
				if (action === "abort") {
					const result = await sendRpcCommand(socketPath, { type: "abort" }, { timeout: 10000 });
					if (!result.response.success) {
						return {
							content: [{ type: "text", text: `Failed: ${result.response.error ?? "unknown error"}` }],
							isError: true,
					...
					return {
						content: [{ type: "text", text: "Abort signal sent" }],
		...
		renderCall(args, theme) {
			...
			const sessionRef = args.sessionName ?? args.sessionId ?? args.project ?? args.projectName ?? args.projectRoot ?? args.tag ?? args.status ?? "...";
		...
		renderResult(result, { expanded }, theme) {
			...
			if (isError || details?.error) {
				const firstText = result.content[0]?.type === "text" ? (result.content[0] as { type: "text"; text: string }).text : undefined;
...
function registerListSessionsTool(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "list_sessions",
		label: "List Sessions",
		description: "List live sessions that expose a control socket, including rich metadata (name, cwd, git project, state, tags/status). Supports optional filters. Use this for discovery only; for the current session id in shell/bash use $PI_SESSION_ID.",
		parameters: Type.Object({
			sessionName: Type.Optional(Type.String({ description: "Filter by exact session name/alias" })),
			project: Type.Optional(Type.String({ description: "Filter by project name/root/projectOverride" })),
			projectName: Type.Optional(Type.String({ description: "Filter by detected project/repository name" })),
			projectRoot: Type.Optional(Type.String({ description: "Filter by detected git/root directory path" })),
			tag: Type.Optional(Type.String({ description: "Filter by custom metadata tag" })),
			status: Type.Optional(Type.String({ description: "Filter by custom metadata status" })),
			metaKey: Type.Optional(Type.String({ description: "Filter sessions that have this custom metadata key" })),
			metaValue: Type.Optional(Type.String({ description: "When metaKey is set, require this stringified metadata value" })),
		}),
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const allSessions = await getLiveSessionInfos();
			const sessions = selectorHasNonIdFields(selector)
				? allSessions.filter((session) => sessionMatchesSelector(session, selector))
				: allSessions;

			if (sessions.length === 0) {
				return {
					content: [{ type: "text", text: allSessions.length === 0 ? "No live sessions found." : "No live sessions matched the filters." }],
					details: { sessions: [], allSessions },
				};
			}

			const currentSessionId = ctx.sessionManager.getSessionId();
			const lines = sessions.map((session) => formatSessionInfo(session, currentSessionId));

			return {
				content: [{ type: "text", text: `Live sessions:\n${lines.join("\n")}` }],
				details: { sessions, count: sessions.length, total: allSessions.length },
			};
		},
	});
}
...
async function maybeHandleStartupControlSend(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	...
	const resolved = await resolveTargetSession(parseControlTargetSelector(target));
	if (!resolved.sessionId) {
		const candidates = resolved.candidates?.map((info) => `- ${formatSessionCandidate(info)}`).join("\n");
		reportStartupControlSend(ctx, candidates ? `${resolved.error}:\n${candidates}` : `Unknown target session: ${target}`, "error");
		return;
	}

	const socketPath = getSocketPath(resolved.sessionId);
...
function registerControlSessionsCommand(pi: ExtensionAPI): void {
	pi.registerCommand("control-sessions", {
		...
		handler: async (_args, ctx) => {
			...
			const sessions = await getLiveSessionInfos();
			const currentSessionId = ctx.sessionManager.getSessionId();
			const lines = sessions.map((session) => formatSessionInfo(session, currentSessionId));
